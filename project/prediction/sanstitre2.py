# -*- coding: utf-8 -*-
"""
Created on Wed Nov  9 13:31:26 2022
...
df1 = pd.read_csv("consommation.csv", delimiter=";", comment="#",
...
df_cons = df_cons.rename(columns={'date': 'Date', 'heure': 'Heure',
                                  'consommation': 'Consommation (MW)'})
df_cons = df_cons.dropna(axis=0)  # supprimer les valeurs manquantes
df_cons = df_cons.set_index('Date')
# print(df_cons.index)
df_cons.index = pd.to_datetime(
    df_cons.index)  # convertir l'objet 'date' de string à datetime
df_cons = df_cons.sort_values(by='Date',
                              ascending=True)  # trier le dataframe dans l'ordre croissant
...
df_cons.plot(style='.', figsize=(20, 5),
             title="Consommation d'électricité en France (MW)")

...
df_cons = df_cons.rename(
    columns={"date": "Date", "heure": "Heure", "consommation": "Consommation (MW)"}
)
...
df_cons.index = pd.to_datetime(
    df_cons.index
)  # convertir l'objet 'date' de string à datetime
...
df_cons.plot(
    style=".", figsize=(20, 5), title="Consommation d'électricité en France (MW)"
)

...
url2 = "https://odre.opendatasoft.com/explore/dataset/eco2mix-national-cons-def/download/?format=csv&disjunctive.nature=true&q=date_heure:%5B2020-12-31T23:00:00Z+TO+2021-11-30T22:59:59Z%5D&timezone=Europe/Berlin&lang=fr&use_labels_for_header=true&csv_separator=%3B"   # noqa
path_target = "./consommation_2021.csv"
path, fname = os.path.split(path_target)
pooch.retrieve(url2, path=path, fname=fname, known_hash=None)
data1 = pd.read_csv(
    "consommation_2021.csv",
    delimiter=";",
    comment="#",
    na_values="n/d",
    parse_dates=["Date"],
...
compa["vrai -3"] = compa.apply(
    lambda f: x(f["Moyenne M1 et M2"], f["vrai"]), axis=1
)
